/**
 * Formulario consolidadoPartidoLista.js cuya funciones es:
 ...
 */
 ...
 function mostrarResultados(fm,evn){
	...
	if(Ext.isEmpty(cmpdivipol.divipol) || Ext.isEmpty(cmpcorporacion.corporacion)){
		...
		if( !Ext.isEmpty(cboxPartido.getValue()) ) {
			parametros.codpartido = cboxPartido.getValue();
		...
		if(!Ext.isEmpty(record)) {
			parametros.codtransmision  = record.data.codTx;
		}
 ...
 function getGrid() {
	...
	var store = new Ext.data.JsonStore({
		url      : URL_SIPREL+'controladores/consolidadoPartidoLista.php',
		autoLoad : false,
		fields   : [
		   // {name : 'divipol'},
		   {name : 'codigo'},
	...
	//La descripcion puede ser el nombre del partido, del candidado o el nombre de la lista
	var grid = new Ext.grid.GridPanel({
		store      : store,
		columns    : [
			// { header : "Divisi&oacute;n Pol&iacute;tica",     width : 115, sortable : true, dataIndex : 'divipol' },
			{ header : "C&oacute;digo",    width : 80,  sortable : true, dataIndex : 'codigo'  },
			{ header : "Descripci&oacute", width : 320, sortable : true, dataIndex : 'partido' },
			{ header : "Votos",            width : 120, sortable : true, dataIndex : 'votos'   }
		],
		tbar         : genBarraExportar('consolidadoPartidoLista'),
		plugins      : buscarPlugin(),
		stripeRows   : true,
		autoScroll   : true,
		border       : true,
