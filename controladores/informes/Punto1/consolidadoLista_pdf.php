<?php
	...
	$pdf->SetSubject('Consolidado Listas');
	$pdf->SetKeywords('Votacion, Listas, Consolidado, Elecciones, Colombia');
	
	$headerstring = utf8_encode("Consolidado Listas");
	...
	$header = array(utf8_encode('DIVISIÓN POLÍTICA'),utf8_encode('CORPORACIÓN'),'NOMBRE','VOTOS');
	$w = array(38, 30, 90,18); //Tamanyo de las columnas
	...
	while($row = ibase_fetch_object($result)) {
		...
		$pdf->Cell($w[1], 6, utf8_encode($row->CORPORACION), 'LR', 0, 'L', $fill,'',$stretch);
	...
	$pdf->Output('consolidadoLista.pdf', 'D');
	unset($pdf);
